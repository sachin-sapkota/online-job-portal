...
const favouritejobs = () => {
  ...
  console.log(fetcher);
  const { data, error } = useSWR('api/employee/getfavjobdetails', fetcher);
  ...
  return (
    <div className="w-full h-screen flex flex-col">
      <div className=" h-[180px] min-h-[180px] w-full flex items-center justify-center bg-gray-200  dark:bg-darkback ">
        <span className="text-5xl font-black">Favourite Jobs</span>
      </div>
...
favouritejobs.getLayout = function getLayout(page) {
  return <DashLayout active="favouritejobs">{page}</DashLayout>;
};
